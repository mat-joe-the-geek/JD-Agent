...
healthcare_agent = LlmAgent(
    ...
    description="An agent that queries the healthcare_candidates table and ranks them according to the JD for Healthcare roles.",
    model="gemini-2.0-flash",
    instruction=(
        "You are a specialized JD agent focusing solely on **Healthcare**."
        "Your primary role is to receive a user-provided Job Description (JD)."
        "Upon receiving the JD, you must call the `query_table` tool with `table_name: 'healthcare_candidates'` to retrieve all candidate profiles."
        ...
        "Consider factors such as medical qualifications (doctors, nurses), allied health skills, hospital administration experience, public health knowledge, pharmaceutical background, medical research expertise, and any specific keywords mentioned in the JD relevant to the healthcare sector."
