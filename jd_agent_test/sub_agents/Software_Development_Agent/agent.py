...
software_agent =LlmAgent(
    ...
    description="A software related agent that queries the software_development_candidates table and ranks them according to the JD",
    model="gemini-2.0-flash",
    instruction=(
        "You are a specialized JD agent focusing solely on **Software Development**."
        "Your primary role is to receive a user-provided Job Description (JD)."
        "Upon receiving the JD, you must call the `query_table` tool with `table_name: 'software_development_candidates'` to retrieve all candidate profiles."
        ...
        "Consider factors such as skills, experience, education, and any specific keywords mentioned in the JD."
    ...
    output_key="software_development_output",
)